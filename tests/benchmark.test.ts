...
async function runBenchmarkTests() {
  console.log('\n================================================================');
  console.log('                 Running Schema v3.0 Performance Benchmark      ');
  console.log('================================================================\n');
  ...
  const sampleRawRev: RawPRRevision = {
    metadata: {
      ...
      state: 'OPEN',
      source: { branch: { name: 'feature/WCE-815' }, commit: { hash: 'a381cc82f8b4' } },
      destination: { branch: { name: 'main' }, commit: { hash: 'b921ef11c990' } }
    },
    commits: [
      { hash: 'a381cc82f8b4', message: 'WCE-815: Add test comment above CRCController' }
    ],
    diffstat: [
    ...
    sourceHash: 'a381cc82f8b4',
    destinationHash: 'b921ef11c990',
    coverage: { metadata: 'complete', commits: 'complete', diffstat: 'complete', diff: 'complete' },
    warnings: []
  };

  // 2. Persist Schema v3.0 Manifest & Check Size
  ...
  console.log(`  📊 Generated Agent Manifest v3.0 Size: ${manifestSizeBytes} bytes (~${(manifestSizeBytes / 1024).toFixed(2)} KB)`);

  assert(result.manifest.schema_version === '3.0', 'Manifest uses Schema v3.0');
  assert(manifestSizeBytes < 2500, 'Small PR Manifest size is under 2.5 KB (Target: 1-2 KB)');
  assert(!manifestJsonStr.includes('siliconstack') && !manifestJsonStr.includes('wec.be'), 'Workspace and repoSlug names excluded from Agent Manifest');
